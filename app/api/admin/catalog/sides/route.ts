...
export async function POST(req:Request){
  ...
  try {
    ...
    if(action==='create'){
      ...
      const row = await prisma.sideOption.create({ data:{ code:code!.trim().toUpperCase(), nameAr:nameAr!.trim(), nameEn:nameEn!.trim(), priceModifier: priceModifier??0 } });
      await prisma.auditLog.create({ data:{ action:'side_create', entity:'SideOption', entityId: row.id } });
    ...
    if(action==='update'){
      ...
      const row = await prisma.sideOption.update({ where:{ id }, data:{ code:code?.trim().toUpperCase(), nameAr:nameAr?.trim(), nameEn:nameEn?.trim(), priceModifier, active } });
      await prisma.auditLog.create({ data:{ action:'side_update', entity:'SideOption', entityId: row.id } });
      return NextResponse.json(row);
    }
    if(action==='delete'){
      if(!id) return NextResponse.json({ error:'missing_id'},{ status:400 });
      await prisma.sideOption.update({ where:{ id }, data:{ active:false } });
      await prisma.auditLog.create({ data:{ action:'side_soft_delete', entity:'SideOption', entityId: id } });
      return NextResponse.json({ ok:true });
    }
    return NextResponse.json({ error:'unknown_action'},{ status:400 });
  } catch(e){
    return NextResponse.json({ error:'side_op_failed'},{ status:500 });
  }
}