      ...
      {labels.map((l, i) => {
        ...
        return (
          <li key={i} className={`flex items-center gap-2 ${active ? "text-ink" : "text-gray-500"}`}>
            <span className={`w-6 h-6 rounded-full grid place-items-center text-white ${done || active ? "bg-gold text-black" : "bg-gray-300"}`}>
              {idx}
            </span>
...
export default function BookHall({ params }: { params: { slug: string } }) {
  ...
  if (!hall) return <div className="container py-10">القاعة غير موجودة.</div>;
  ...
  const [men, setMen] = useState<number>(100);
  const [women, setWomen] = useState<number>(100);

  // Step 2: add-ons (selected ids; selection happens on vendor pages too)
  ...
  const holdHours = 48; // Admin-config later
  ...
  function handleSubmit() {
    ...
    addItem({
      ...
      price: hall.basePrice, // base rent; services are separate items
      qty: 1,
      startDate,
      days,
      session,
    ...
    // Redirect to /cart to review hall + any service items added from other pages
    router.push("/cart");
  }

  // Lightweight estimate to show sidebar totals (unchanged by cart)
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
          ...
          <div className="flex flex-col gap-4">
            ...
            {step === 2 && (
              <div className="card p-4">
                ...
                {/* Quick suggestions (checkboxes stay as a light, optional UI) */}
                ...
                {/* Deep links to real vendor sources (best practice) */}
                <div className="flex flex-wrap gap-2 mt-3">
                  <a href="/decor" className="btn btn-ghost">
                    <i className="fa-solid fa-wand-magic-sparkles" /> الديكور
                  </a>
                  <a href="/photography" className="btn btn-ghost">
                    <i className="fa-solid fa-camera" /> التصوير
                  </a>
                  <a href="/catering" className="btn btn-ghost">
                    <i className="fa-solid fa-utensils" /> الكيترنغ
                  </a>
                  <a href="/catering" className="btn btn-ghost">
                    <i className="fa-solid fa-mug-hot" /> القهوة
                  </a>
                </div>
              ...
              <div className="card p-4">
                ...
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <div className="card p-3">
                    <b>القاعة:</b> <span className="text-gray-700">{hall.name}</span>
                    <br />
                    <b>الفترة:</b> <span className="text-gray-700">{session}</span>
                    <br />
                    <b>التواريخ:</b>{" "}
                    <span className="text-gray-700">
                      {startDate || "—"} → {endDate || startDate || "—"} ({days} يوم)
                    </span>
                    <br />
                    <b>الأعداد:</b>{" "}
                    <span className="text-gray-700">
                    ...
                    <br />
                    <b>إضافات (سريعة):</b>{" "}
                    <span className="text-gray-700">{addons.length ? addons.join("، ") : "لا يوجد"}</span>
                  </div>
                  <div className="card p-3">
                    <b>ملخص تقديري:</b>
                    <div className="mt-1">
                      إيجار القاعة: <b>{formatPrice(hall.basePrice)} ر.س</b>
                    </div>
                    <div>
                      الضيافة (تقديري): <b>{formatPrice(cateringEstimate)} ر.س</b>
                    </div>
                    <div>
                      رسوم المنصة (5%): <b>{formatPrice(platformFee)} ر.س</b>
                    </div>
                    <div>
                      الضريبة (15%): <b>{formatPrice(vat)} ر.س</b>
                    </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {step < 3 && (
                <>
                  ...
                  {step === 2 && (
                    <button className="btn btn-ghost" onClick={skipAddons}>
                      تخطي الإضافات
                    </button>
                  )}
                </>
          ...
          <aside className="card p-4 h-max sticky top-24 space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <b>{formatPrice(hall.basePrice)} ر.س</b>
            </div>
            <div className="flex justify-between">
              <span>الضيافة (تقديري)</span>
              <b>{formatPrice(cateringEstimate)} ر.س</b>
            </div>
            <div className="flex justify-between">
